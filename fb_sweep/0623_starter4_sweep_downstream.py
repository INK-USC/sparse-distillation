...
def get_grid(args):
    ...
    grid += [
        hyperparam("--restore-file", "/data/home/yeqy/src/fairseq/checkpoints/0623-imdb-bigdata-smalldict-bsz2k-1gpu-2048x1-dim100-200k/checkpoint_last.pt"),
        ...
        hyperparam("--encoder-embed-dim", 100),
    ]

    grid += [
        hyperparam("--weight-decay", [0.0, 1e-3, 1e-2, 1e-1], save_dir_key=lambda val: f"wd{val}"),
        hyperparam("--lr", [3e-3, 1e-3, 3e-4, 1e-4, 3e-5, 1e-5], save_dir_key=lambda val: f"lr{val}"),
    ]
    ...
    grid += [
        hyperparam("--batch-size", [16, 32, 64, 128], save_dir_key=lambda val: f"bsz{val}"),
    ]

    grid += [
        hyperparam("--clip-norm", [0.0, 0.1, 1.0], save_dir_key=lambda val: f"clp{val}"),
    ]
