...
@register_task("feature_based_sentence_prediction")
class FeatureBasedSentencePredictionTask(SentencePredictionTask):
    def add_args(parser):
    ...
    def load_dataset(self, split, combine=False, **kwargs):
        ...
        if self.args.arch not in ["distilroberta_base"]:
            input0 = StripTokenDataset(input0, id_to_strip=self.source_dictionary.eos())

        if self.args.arch in ["distilroberta_base"]:
            input0 = PrependTokenDataset(input0, self.args.init_token)
            input0 = maybe_shorten_dataset(
                input0,
                split,
        ...
        #     actual_size = len(input0) - self.args.exclude_last_instances
        #     input0 = TruncateDataset(input0, actual_size)
        src_tokens = input0

        # dropout features (for regularization) + truncate if there are too many features
        if self.args.feature_dropout > 0:
        ...
        # if split == "train" and self.args.exclude_last_instances > 0:
        #     actual_size = len(label_dataset) - self.args.exclude_last_instances
        #     label_dataset = TruncateDataset(label_dataset, actual_size)
        ...
        if self.args.distillation_target:
            ...
            if os.path.exists(label_path + ".npy"):
                ...
                # if split == "train" and self.args.exclude_last_instances > 0:
                #     actual_size = len(distillation_target) - self.args.exclude_last_instances
                #     distillation_target = TruncateDataset(distillation_target, actual_size)
    ...
    def build_model(self, args):
        ...
        if args.arch not in ["distilroberta_base"]:
